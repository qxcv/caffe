...
/**
 * @brief Computes the Manhattan (L1) loss @f$
 ...
 *        \right| \right|_1 @f$
 *
 * This is a nice alternative to the EuclideanLossLayer for regression tasks in
 * which you don't want to penalise outliers too severely. Particularly useful
 * for object localisation tasks.
 */
template <typename Dtype>
class ManhattanLossLayer : public LossLayer<Dtype> {
 public:
  explicit ManhattanLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "ManhattanLoss"; }
  /**
   * Like EuclideanLossLayer, ManhattanLossLayer can backpropagate to both
 ...
 protected:
  ...
  /**
   * @brief Computes the Manhattan error gradient w.r.t. the inputs. Smoothly
   * passes from -1 to 1 as it moves over the origin.
   *
   * Like EuclideanLossLayer, ManhattanLossLayer \b can compute gradients with
   ...
   *      gradients @f$
   *        \frac{\partial E}{\partial \hat{y}} =
   *            \frac{1}{n} \sum\limits_{n=1}^N \frac{\hat{y}_n -
   *            y_n}{\left|\hat XXX +
   *            \epsilon}
   *      @f$ if propagate_down[0]
   *   -# @f$ (N \times C \times H \times W) @f$
   *      the targets @f$y@f$; Backward fills their diff with gradients
   *      @f$ \frac{\partial E}{\partial y} =
   *          \frac{1}{n} \sum\limits_{n=1}^N (y_n - \hat{y}_n) XXX
   *      @f$ if propagate_down[1]
   */
  virtual void Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
...
/**
 * @brief Computes consistency loss using the L1 norm.
 *
...
class ConsistencyLossLayer : public LossLayer<Dtype> {
 public:
  ...
  virtual inline bool AllowForceBackward(const int bottom_index) const {
    // We can only go backwards to the regressor output (not the flow)
